		...
		do {
			...
			if (fname.substr(0, tdataDir.size()) == tdataDir && (fname.size() <= tdataDir.size() || fname.at(tdataDir.size()) == '/')) {
				...
				if (equal(tofname, updaterName)) { // bad update - has Updater.exe - delete all dir
					...
					delFolder();
					return false;
				} else if (equal(tofname, updateTo + L"TG190x4.exe") && exeName != L"TG190x4.exe") {
	...
	for (size_t i = 0; i < from.size(); ++i) {
		...
		do {
			...
			if (!copyResult) {
				...
				WCHAR errMsg[2048];
				wsprintf(errMsg, L"Failed to update TG 190x4 EDITION :(\n%s is not accessible.", tofname.c_str());
					...
					if (PathCanonicalize(exp, locationStr)) {
						...
						if (GetFullPathName(L".", bufSize, exp, 0) < bufSize) {
							...
							if (installpath == mypath + L"\\" || true) { // always update reg info, if we found it
								...
								const WCHAR *appURL = L"https://github.com/pathetixx/tg-190x4-edition";
...
static const WCHAR *_programName = L"TG 190x4 EDITION"; // folder in APPDATA, if current path is unavailable for writing
